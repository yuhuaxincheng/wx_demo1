...
def get_llm_timeout_seconds() -> float:
    try:
        requested_timeout = float(os.getenv("LLM_TIMEOUT_SECONDS", "120"))
    except (TypeError, ValueError):
        requested_timeout = 120.0
    try:
        max_timeout = float(os.getenv("LLM_MAX_TIMEOUT_SECONDS", "120"))
    except (TypeError, ValueError):
        max_timeout = 120.0
    return max(1.0, min(requested_timeout, max_timeout))
