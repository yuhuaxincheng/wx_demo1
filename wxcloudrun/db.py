...
def create_recommendation_task(req: RecommendRequest) -> Optional[str]:
    if not db_enabled():
        return None
    ...
    try:
        ...
        with _connect(use_database=True) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    INSERT INTO recommendation_tasks (task_id, status, request_json)
                    VALUES (%s, %s, %s)
                    """,
                    (
                        task_id,
                        "pending",
                        json.dumps(_model_to_dict(req), ensure_ascii=False),
                    ),
                )
        return task_id
    except Exception:
...
def update_recommendation_task(
    task_id: str,
    status: str,
    response: Optional[RecommendResponse] = None,
    error_text: str = "",
) -> bool:
    if not db_enabled() or not task_id:
        return False
    ...
    try:
        ...
        response_json = None
        if response:
            response_json = json.dumps(_model_to_dict(response), ensure_ascii=False)
        with _connect(use_database=True) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    UPDATE recommendation_tasks
                    SET status = %s, response_json = %s, error_text = %s
                    WHERE task_id = %s
                    """,
                    (status, response_json, error_text or "", task_id),
                )
        return True
    except Exception:
        return False
...
def get_recommendation_task(task_id: str) -> Optional[Dict[str, Any]]:
    if not db_enabled() or not task_id:
        return None
    ...
    try:
        ...
        with _connect(use_database=True) as conn:
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    SELECT task_id, status, created_at, updated_at, request_json,
                           response_json, error_text
